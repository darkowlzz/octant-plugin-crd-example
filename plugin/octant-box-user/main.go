...
func main() {
	...
	p, err := service.Register(pluginName, "box-user octant plugin", capabilities, options...)
	if err != nil {
		log.Fatal(err)
	}

	// The plugin can log and the log messages will show up in Octant.
	log.Printf("octant-box-user is starting")
	p.Serve()
}
...
func handleTab(request *service.PrintRequest) (plugin.TabResponse, error) {
	...
	contents := component.NewMarkdownText("user content from a *plugin*")
	...
	tab := component.NewTabWithContents(*layout.ToComponent("Box User Details"))
...
func handlePrint(request *service.PrintRequest) (plugin.PrintResponse, error) {
	...
	boxCard := component.NewCard(component.TitleFromString(fmt.Sprintf("User Output for %s", u.GetName())))
	boxCard.SetBody(component.NewMarkdownText("This output was generated for user by _octant-box-user_"))
...
func initRoutes(router *service.Router) {
	router.HandleFunc("*", func(request service.Request) (component.ContentResponse, error) {
		...
		text := component.NewText("Box User Dashboard")
		title = component.Title(text)

		contentResponse := component.NewContentResponse(title)

		card1 := component.NewCard(component.TitleFromString("Card 1"))
		card1Body := component.NewText("Lorem ipsum dolor sit amet, consectetur.")
		card1.SetBody(card1Body)

		card2 := component.NewCard(component.TitleFromString("Card 2"))
		card2Body := component.NewText("Donec bibendum pulvinar ex, at tempus magna consectetur quis.")
		card2.SetBody(card2Body)

		cardList := component.NewCardList("A list of cards")
